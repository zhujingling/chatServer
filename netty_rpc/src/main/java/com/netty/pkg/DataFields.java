...
public class DataFields implements Serializable {
	...
	public void put(byte[] val, int offset, int datalen) {
		...
		DataField df = new DataField();
		df.dataType = DataField.DATATYPE_BYTEARRAY;
		if (null == val || offset < 0 || datalen > val.length
				|| offset + datalen > val.length) {
			df.length = (short) 0;
			df.data = new byte[0];
		} else {
	...
	public void put(byte[] val1, byte[] val2) {
		...
		if (null == val1 || null == val2) {
			df.length = (short) ((val1 != null ? val1.length : 0) + (val2 != null ? val2.length
					: 0));
			df.data = new byte[df.length];
			if (val1 != null) {
			...
			if (val2 != null) {
				System.arraycopy(val2, 0, df.data, (val1 != null ? val1.length
						: 0), val2.length);
			}
		} else {
